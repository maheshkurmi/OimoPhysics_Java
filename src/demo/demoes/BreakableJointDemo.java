...
public class BreakableJointDemo extends DemoBase {
	...
	public void init(World world, DemoRenderer renderer, UserInput input, ViewInfo viewInfo) {
		...
		chain[0]=Utils.addSphere(world, new Vec3(0, 6, 0), 0.3, true);
		for (int i=0;i<12;i++) {
			if (i % 2 == 0) {
				chain[i+1]=Utils.addSphere(world, new Vec3((i + 1) * 0.4, 6, 0), 0.25, false);
			} else {
				chain[i+1]=Utils.addBox(world, new Vec3((i + 1) * 0.4, 6, 0), new Vec3(0.25, 0.25, 0.25), false);
			}
		...
		for (int i=1;i<chain.length;i++) {
			...
			Joint joint = Utils.addSphericalJoint(world, chain[i - 1], chain[i], center);
